...
func SetupRouter(cfg *config.Config, c *provider.Container) *gin.Engine {
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		user.Use(UserJWTAuthMiddleware(cfg.UserJWT.SecretKey, c.UserRepo))
		{
			...
			user.POST("/sites/open/preview", publicHandler.PreviewOpenSite)
			user.POST("/sites/open", publicHandler.OpenSite)
			user.GET("/sites/prices", publicHandler.ListMySitePrices)
			user.PUT("/sites/prices", publicHandler.SetMySitePrice)
			user.GET("/sites/me/summary", publicHandler.GetMySiteSummary)
			user.GET("/sites/me/orders", publicHandler.ListMySiteOrderSummaries)
			user.GET("/sites/me/profit-ledgers", publicHandler.ListMySiteProfitLedgers)
			user.POST("/sites/me/withdraws", publicHandler.ApplySiteWithdraw)
			user.GET("/sites/me/withdraws", publicHandler.ListMySiteWithdraws)
		...
		admin := apiV1.Group("/admin")
		{
			...
			authorized := admin.Use(JWTAuthMiddleware(cfg.JWT.SecretKey, c.AdminRepo), AdminRBACMiddleware(c.AuthzService))
			{
				...
				authorized.GET("/settings/site-open", adminHandler.GetSiteOpenSettings)
				authorized.PUT("/settings/site-open", adminHandler.UpdateSiteOpenSettings)
				authorized.GET("/subsites/settings", adminHandler.GetSiteOpenSettings)
				authorized.PUT("/subsites/settings", adminHandler.UpdateSiteOpenSettings)
				authorized.GET("/subsites/suffixes", adminHandler.ListSubsiteSuffixes)
				authorized.POST("/subsites/suffixes", adminHandler.CreateSubsiteSuffix)
				authorized.PUT("/subsites/suffixes/:id", adminHandler.UpdateSubsiteSuffix)
				authorized.DELETE("/subsites/suffixes/:id", adminHandler.DeleteSubsiteSuffix)
				authorized.GET("/subsites", adminHandler.ListSubsites)
				authorized.PUT("/subsites/:id/status", adminHandler.UpdateSubsiteStatus)
				...
				authorized.GET("/sites/withdraws", adminHandler.ListSiteWithdraws)
				authorized.POST("/sites/withdraws/:id/reject", adminHandler.RejectSiteWithdraw)
				authorized.POST("/sites/withdraws/:id/pay", adminHandler.PaySiteWithdraw)
				authorized.GET("/subsites/withdraws", adminHandler.ListSiteWithdraws)
				authorized.POST("/subsites/withdraws/:id/reject", adminHandler.RejectSiteWithdraw)
				authorized.POST("/subsites/withdraws/:id/pay", adminHandler.PaySiteWithdraw)
