...
type AdminSiteListFilter struct {
	Page     int
	PageSize int
	Status   string
}

type AdminSiteSuffixInput struct {
	Suffix    string
	IsEnabled bool
	SortOrder int
}
...
func (s *SiteService) ListAdminSites(filter AdminSiteListFilter) ([]models.Site, int64, error) {
	if s == nil || s.repo == nil {
		return []models.Site{}, 0, nil
	}
	return s.repo.List(filter.Page, filter.PageSize, filter.Status)
}

func (s *SiteService) UpdateAdminSiteStatus(id uint, status string) (*models.Site, error) {
	...
	if status != models.SiteStatusActive && status != models.SiteStatusDisabled {
		return nil, ErrSiteStatusInvalid
	}
	row.Status = status
...
func (s *SiteService) ListAdminSuffixes() ([]models.SiteDomainSuffix, error) {
	if s == nil || s.suffixRepo == nil {
		return []models.SiteDomainSuffix{}, nil
	}
	return s.suffixRepo.ListAll()
}

func (s *SiteService) CreateAdminSuffix(input AdminSiteSuffixInput) (*models.SiteDomainSuffix, error) {
	if s == nil || s.suffixRepo == nil {
		return nil, ErrSiteSuffixInvalid
	}
	suffix := normalizeSuffix(input.Suffix)
	if suffix == "" {
		return nil, ErrSiteSuffixInvalid
	}
	if exists, err := s.suffixRepo.GetBySuffix(suffix); err != nil {
		return nil, err
	} else if exists != nil {
		return nil, ErrSiteDomainExists
	}
	now := time.Now()
	row := &models.SiteDomainSuffix{Suffix: suffix, IsEnabled: input.IsEnabled, SortOrder: input.SortOrder, CreatedAt: now, UpdatedAt: now}
	if err := s.suffixRepo.Create(row); err != nil {
		return nil, err
	}
	return row, nil
}

func (s *SiteService) UpdateAdminSuffix(id uint, input AdminSiteSuffixInput) (*models.SiteDomainSuffix, error) {
	if s == nil || s.suffixRepo == nil || id == 0 {
		return nil, ErrNotFound
	}
	row, err := s.suffixRepo.GetByID(id)
	if err != nil {
		return nil, err
	}
	if row == nil {
	...
	if exists, err := s.suffixRepo.GetBySuffix(suffix); err != nil {
		return nil, err
	} else if exists != nil && exists.ID != id {
		return nil, ErrSiteDomainExists
	}
	row.Suffix = suffix
	...
	row.UpdatedAt = time.Now()
	if err := s.suffixRepo.Update(row); err != nil {
		return nil, err
	}
	return row, nil
}

func (s *SiteService) DeleteAdminSuffix(id uint) error {
	if s == nil || s.suffixRepo == nil || id == 0 {
		return ErrNotFound
	}
	row, err := s.suffixRepo.GetByID(id)
...
var (
	...
	ErrSiteStatusInvalid  = errors.New("site status invalid")
)
